...
*/
static unsigned short getTiffHeaderSeparator(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
*/
static unsigned int getTiffHeaderOffset(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
@return 取得した区切り文字（見つからなかった場合は 0x0000 ）
*/
static unsigned short getIFDCount(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
*/
static EntryTag_t getEntryTag(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
*/
static EntryType_t getEntryType(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
*/
static EntryCount_t getEntryCount(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
*/
static EntryValueOffset_t getEntryValueOffset(JpegBuffer *src, unsigned long seek, JACIC_ENDIANS jacicEndian)
{
...
static int takeDateTimeFromAPP1(JpegBuffer *src, HashBuffer **dateTimeOriginalBin, HashBuffer **subSecTimeOriginalBin, unsigned long *seek)
{
    ...
    *seek = tmpSeek + app1Length;
    ...
    for(i = 0; i < exifIFDCount; ++i)
    {
        ...
        /* 仮想走査位置をエントリのサイズ分進める */
        entrySize = getByteSizeOfEntry(entry);
...
FINALIZE:
    ...
    /* メモリ解放 */
    _SECURE_FREE(dateTimeOriginalBuffer);
    _SECURE_FREE(subSecTimeOriginalBuffer);
